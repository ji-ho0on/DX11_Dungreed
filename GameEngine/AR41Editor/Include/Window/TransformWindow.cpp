...
void CTransformWindow::SetSelectComponent(CSceneComponent* Component)
{
	...
	if (Component)
	{
		...
		{
			SetPos(Component->GetRelativePos());
			SetScale(Component->GetRelativeScale());
			SetRotation(Component->GetRelativeRot());
		}

		else
		{
			SetPos(Component->GetWorldPos());
			SetScale(Component->GetWorldScale());
			SetRotation(Component->GetWorldRot());
		}

		SetPivot(Component->GetPivot());
	}
}
...
bool CTransformWindow::Init()
{
	...
	m_Pivot[0]->SetInputCallback<CTransformWindow>(this, &CTransformWindow::PivotXCallback);
	m_Pivot[1]->SetInputCallback<CTransformWindow>(this, &CTransformWindow::PivotYCallback);
	m_Pivot[2]->SetInputCallback<CTransformWindow>(this, &CTransformWindow::PivotZCallback);
...
void CTransformWindow::PivotXCallback()
{
	if (m_SelectComponent)
	{
		m_SelectComponent->SetPivot(m_Pivot[0]->GetFloat(), m_Pivot[1]->GetFloat(), m_Pivot[2]->GetFloat());
	}
}
