...
bool CResourceManager::CreateMesh(CScene* Scene, EMeshType Type,	const std::string& Name,
...
{
	return m_MeshManager->CreateMesh(Scene, Type, Name, VtxData, Size, Count, VtxUsage, Primitive,
...
CColliderLine2DConstantBuffer* CResourceManager::GetColliderLine2DCBuffer() const
{
	return m_ShaderManager->GetColliderLine2DCBuffer();
}
...
{
	return m_AnimationManager->AddAnimationSequence2DFrame(Name, Start, End);
}

bool CResourceManager::AddAnimationSequence2DFrame(const std::string& Name, float StartX, float StartY, float EndX, float EndY)
{
	return m_AnimationManager->AddAnimationSequence2DFrame(Name, StartX, StartY, EndX, EndY);
}
...
{
	return m_AnimationManager->AddAnimationSequence2DFrameAll(Name, Count, Start, End);
}
...
{
	return m_AnimationManager->AddAnimationSequence2DFrameAll(Name, Count, StartX, StartY, EndX, EndY);
}
