...
bool CTexture::CreateResourceArray()
{
	...
	if (FAILED(ImageArray->Initialize2D(m_vecTextureInfo[0]->Image->GetMetadata().format,
		m_vecTextureInfo[0]->Image->GetMetadata().width, m_vecTextureInfo[0]->Image->GetMetadata().height, Count, MipLevel)))
		return false;
	...
	if (FAILED(DirectX::CreateTextureEx(CDevice::GetInst()->GetDevice(), ImageArray->GetImages(), ImageArray->GetImageCount(), ImageArray->GetMetadata(),
		D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0, DirectX::CREATETEX_FLAGS::CREATETEX_DEFAULT,	(ID3D11Resource**)&Texture)))
		return false;
