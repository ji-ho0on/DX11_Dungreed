...
void CGraphicShader::SetShader()
{
	...
	// IA(InputAssembler, 입력병합기)에 InputLayout을 지정
	CDevice::GetInst()->GetContext()->IASetInputLayout(m_InputLayout);
}

void CGraphicShader::AddInputDesc(const char* SemanticName,	unsigned int SemanticIndex, DXGI_FORMAT Fmt,
	unsigned int Slot, unsigned int Size, D3D11_INPUT_CLASSIFICATION SlotClass,	unsigned int InstanceDataStepRate)
{
	D3D11_INPUT_ELEMENT_DESC	Desc = {};

	Desc.SemanticName = SemanticName;

	// SemanticIndex는 Position 레지스터라면 0을 입력하면 POSITION의 0번 레지스터를 의미한다. 
...
{
	// 이미 Shader가 생성되어 있을수 있으므로 Release 해줌
	SAFE_RELEASE(m_VS);
	SAFE_RELEASE(m_VSBlob);
	...
	if (FAILED(D3DCompileFromFile(FullPath, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, EntryName, "vs_5_0", Flag, 0, &m_VSBlob, &ErrorBlob)))
	{
...
bool CGraphicShader::LoadPixelShader(const char* EntryName,	const TCHAR* FileName, const std::string& PathName)
{
	...
	// ErrorBlob은 에러가 발생했을 경우 에러의 내용을 저장하기 위한 버퍼이다.
	if (FAILED(D3DCompileFromFile(FullPath, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, EntryName, "ps_5_0", Flag, 0, &m_PSBlob, &ErrorBlob)))
	{
		OutputDebugStringA((char*)ErrorBlob->GetBufferPointer());
		OutputDebugStringA("\n");
	...
	// 위에서 컴파일한 코드를 이용해서 PixelShader 객체를 만들어낸다.
...
bool CGraphicShader::LoadHullShader(const char* EntryName, const TCHAR* FileName, const std::string& PathName)
{
	...
	// ErrorBlob은 에러가 발생했을 경우 에러의 내용을 저장하기 위한 버퍼이다.
	if (FAILED(D3DCompileFromFile(FullPath, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, EntryName, "hs_5_0", Flag, 0, &m_HSBlob, &ErrorBlob)))
	{
		OutputDebugStringA((char*)ErrorBlob->GetBufferPointer());
		OutputDebugStringA("\n");
	...
	// 위에서 컴파일한 코드를 이용해서 HullShader 객체를 만들어낸다.
...
bool CGraphicShader::LoadDomainShader(const char* EntryName, const TCHAR* FileName, const std::string& PathName)
{
	...
	// ErrorBlob은 에러가 발생했을 경우 에러의 내용을 저장하기 위한 버퍼이다.
	if (FAILED(D3DCompileFromFile(FullPath, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, EntryName, "ds_5_0", Flag, 0, &m_DSBlob, &ErrorBlob)))
	{
		OutputDebugStringA((char*)ErrorBlob->GetBufferPointer());
		OutputDebugStringA("\n");
	...
	// 위에서 컴파일한 코드를 이용해서 DomainShader 객체를 만들어낸다.
...
bool CGraphicShader::LoadGeometryShader(const char* EntryName, const TCHAR* FileName, const std::string& PathName)
{
	...
	// ErrorBlob은 에러가 발생했을 경우 에러의 내용을 저장하기 위한 버퍼이다.
	if (FAILED(D3DCompileFromFile(FullPath, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, EntryName, "gs_5_0", Flag, 0, &m_GSBlob, &ErrorBlob)))
	{
		OutputDebugStringA((char*)ErrorBlob->GetBufferPointer());
		OutputDebugStringA("\n");
	...
	// 위에서 컴파일한 코드를 이용해서 GeometryShader 객체를 만들어낸다.
