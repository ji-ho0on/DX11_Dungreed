...
void CTransform::InheritRotation(bool Current)
{
	...
	// 단, x, y, z 축 회전에 모두 따로 영향을 받을지 말지를 정할 수 있게 해두었으므로 각 축마다 계산한다.
	if (m_Parent)
	{
	...
	// 축은 위치가 필요 없는 방향만 존재하므로 위치에 영향을 안받고 회전에만 영향을 받게 곱해주면 된다.
	// 즉, x, y, z 가 있을 경우 4x4 행렬과 곱하기 위해 w를 0으로 만든다면 행렬의 41, 42, 43에 영향을 안받고 곱해지게 되므로
	// 위치에 영향을 안받을때는 w를 0으로, 영향을 받을때는 w를 1로 해서 곱하면 된다.
	for (int i = 0; i < AXIS_MAX; ++i)
	{
...
void CTransform::InheritWorldRotation(bool Current)
{
	...
	if (m_Parent)
	{
		// x, y, z 축 회전중 하나라도 부모에 영향을 받고 현재 트랜스폼이 아닌
		// 회전이 일어난 트랜스폼의 자식 트랜스폼일 경우 위치가 변경이 되어야 한다.
		if ((m_InheritRotX || m_InheritRotY || m_InheritRotZ) && !Current)
...
void CTransform::InheritWorldParentRotationPos()
{
	...
	if (m_Parent)
	{
		...
		if (m_InheritRotX || m_InheritRotY || m_InheritRotZ)
		{
			// 부모로부터 회전한 위치로 적용이 되기 때문에 월드위치를 강제할 경우 해당 위치로부터 현재의 상대적인 위치를 구한 후에
			// 부모 회전의 역회전을 이용하여 최종 상대적인 위치를 구해주어야 한다.
...
void CTransform::PostUpdate(float DeltaTime)
{
	...
	if (m_Is2D)
	{
		...
		float	z = (y - (m_MinY - 1.f)) / (SizeY / 1000.f) + m_2DZOffset;
	...
	// x, y, z 축으로 회전을 하거나 아니면 임의의 축을 이용해서 회전을 하거나 둘중 하나의 경우만 사용을 한다.
	...
	// 크기, 위치, 회전 정보에 대한 업데이트가 끝났으므로 false로 바꿔준다.
	m_UpdateScale = false;
	m_UpdateRot = false;
	m_UpdateRotAxis = false;
	m_UpdatePos = false;
...
{
	// 상수버퍼에 계산된 월드 행렬을 지정한다.
	m_CBuffer->SetWorldMatrix(m_matWorld);

	// CameraManager에서 CurrentCamera의 View행렬과 Projection행렬을 받아온다.
...
{
	// PostUpdate에서 자동으로 계산될 떄가 아니라 따로 계산이 필요할 때
	m_matWorld = m_matScale * m_matRot * m_matPos;
}
...
void CTransform::Save(FILE* File)
{
	...
	fwrite(&m_2DZOffset, sizeof(float), 1, File);
...
void CTransform::Load(FILE* File)
{
	...
	fread(&m_2DZOffset, sizeof(float), 1, File);
