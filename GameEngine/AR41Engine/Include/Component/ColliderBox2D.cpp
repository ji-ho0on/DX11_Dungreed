...
void CColliderBox2D::PostUpdate(float DeltaTime)
{
	...
	m_Min.x = GetWorldPos().x - GetPivot().x * Size.x;
	m_Min.y = GetWorldPos().y - GetPivot().y * Size.y;

	m_Max.x = GetWorldPos().x + (1.f - GetPivot().x) * Size.x;
	m_Max.y = GetWorldPos().y + (1.f - GetPivot().y) * Size.y;
...
bool CColliderBox2D::Collision(CCollider* Dest)
{
	...
	case ECollider2D_Type::Line2D:
		Result = CCollisionManager::GetInst()->CollisionBox2DToLine2D(HitPoint, this, (CColliderLine2D*)Dest);
		break;
	}
