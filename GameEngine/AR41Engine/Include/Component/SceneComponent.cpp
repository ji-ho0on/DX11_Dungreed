...
bool CSceneComponent::DeleteChild(const std::string& Name)
{
	...
	for (size_t i = 0; i < Size; ++i)
	{
		if (m_vecChild[i]->GetName() == Name)
		{
			...
			// 지울 Child의 Child들 부터 모두 지워준다.
			int ChildCount = (*iter)->GetChildCount();

			for (int i = 0; i < ChildCount; ++i)
			{
				(*iter)->DeleteChild((*iter)->GetChildName(i));
			}
...
void CSceneComponent::Set2DZOffset(float In2DZoffset)
{
	m_Transform->Set2DZOffset(In2DZoffset);
}
