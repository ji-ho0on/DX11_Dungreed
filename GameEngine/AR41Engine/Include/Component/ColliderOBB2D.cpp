...
bool CColliderOBB2D::Init()
{
	...
	{
		m_Mesh = CResourceManager::GetInst()->FindMesh("LBBox2DLineMesh");
	}
...
void CColliderOBB2D::PostUpdate(float DeltaTime)
{
	...
	//m_Info.Center.x = GetWorldPos().x - GetPivot().x * (Size.x * 2) + Size.x;
	//m_Info.Center.y = GetWorldPos().y - GetPivot().y * (Size.y * 2) + Size.y;

	//m_Info.Center = Vector2(GetWorldPos().x, GetWorldPos().y) - m_Info.Axis[AXIS2D_X] * (Vector2(GetPivot().x, GetPivot().y) * Size * 2);

	m_Info.Center = Vector2(GetWorldPos().x, GetWorldPos().y) - m_Info.Axis[AXIS2D_X] * GetPivot().x * (m_Info.Length[AXIS2D_X] * 2.f) - m_Info.Axis[AXIS2D_Y] * GetPivot().y * (m_Info.Length[AXIS2D_Y] * 2.f)
		+ m_Info.Axis[AXIS2D_X] * m_Info.Length[AXIS2D_X] + m_Info.Axis[AXIS2D_Y] * m_Info.Length[AXIS2D_Y];


	Vector2	Pos[4];

	Pos[0] = m_Info.Center - m_Info.Axis[AXIS2D_X] * m_Info.Length[AXIS2D_X] + m_Info.Axis[AXIS2D_Y] * m_Info.Length[AXIS2D_Y];

	Pos[1] = m_Info.Center + m_Info.Axis[AXIS2D_X] * m_Info.Length[AXIS2D_X] + m_Info.Axis[AXIS2D_Y] * m_Info.Length[AXIS2D_Y];

	Pos[2] = m_Info.Center - m_Info.Axis[AXIS2D_X] * m_Info.Length[AXIS2D_X] - m_Info.Axis[AXIS2D_Y] * m_Info.Length[AXIS2D_Y];

	Pos[3] = m_Info.Center + m_Info.Axis[AXIS2D_X] * m_Info.Length[AXIS2D_X] - m_Info.Axis[AXIS2D_Y] * m_Info.Length[AXIS2D_Y];
...
bool CColliderOBB2D::Collision(CCollider* Dest)
{
	...
	case ECollider2D_Type::Line2D:
		Result = CCollisionManager::GetInst()->CollisionOBB2DToLine2D(HitPoint, this, (CColliderLine2D*)Dest);
		break;
	}
