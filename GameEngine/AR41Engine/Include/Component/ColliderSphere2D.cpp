...
void CColliderSphere2D::PostUpdate(float DeltaTime)
{
	...
	//m_Info.Center.x = GetRelativePos().x;
	//m_Info.Center.y = GetRelativePos().y;

	//float Scale = GetWorldScale().x > GetWorldScale().y ? GetWorldScale().x : GetWorldScale().y;
	float Scale = GetRelativeScale().x > GetRelativeScale().y ? GetRelativeScale().x : GetRelativeScale().y;
...
bool CColliderSphere2D::Collision(CCollider* Dest)
{
	...
	case ECollider2D_Type::Line2D:
		Result = CCollisionManager::GetInst()->CollisionSphere2DToLine2D(HitPoint, this, (CColliderLine2D*)Dest);
		break;
	}
