...
bool CAnimation2D::AddAnimation(const std::string& Name, const std::string& SequenceName, float PlayTime, float PlayScale, bool Loop, bool Reverse)
{
...
void CAnimation2D::GetAllAnimationName(std::vector<std::string>& vecName)
{
	auto iter = m_mapAnimation.begin();
	auto iterEnd = m_mapAnimation.end();
	...
	for (; iter != iterEnd; ++iter)
	{
		vecName.push_back(iter->second->GetName());//GetAnimationSequence()->GetName());
	}
}
