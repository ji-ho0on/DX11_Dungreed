...
#include "Resource/Shader/MouseConstantBuffer.h"
...
CInput::~CInput()
{
	...
	SAFE_DELETE(m_MouseCBuffer);
}

bool CInput::SetMouseTexture(const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
	m_MouseMaterial->SetTexture(0, 0, (int)EShaderBufferType::Pixel, Name, FileName, PathName);

	return true;
}

bool CInput::SetMouseTextureFullPath(const std::string& Name, const TCHAR* FullPath)
{
	m_MouseMaterial->SetTexture(0, 0, (int)EShaderBufferType::Pixel, Name, FullPath);
...
bool CInput::Init(HINSTANCE hInst, HWND hWnd)
{
	...
	m_MouseCBuffer = new CMouseConstantBuffer;
	m_MouseCBuffer->Init();

	m_MouseMesh = CResourceManager::GetInst()->FindMesh("CenterUVRect");

	m_MouseMaterial = CResourceManager::GetInst()->FindMaterial("MouseMaterial");

	m_MouseSize = Vector2(76.f, 76.f);
...
void CInput::Render()
{
	...
	matScale.Scaling(m_MouseSize);
	matTranslate.Translation(m_MousePos);

	Matrix  matWorld = matScale * matTranslate;

	Matrix  matWVP = matWorld * matProj;

	m_MouseCBuffer->SetWVP(matWVP);

	m_MouseCBuffer->UpdateBuffer();

	if (m_MouseMaterial && m_MouseMesh)
	{
		m_MouseMaterial->SetMaterial();

		m_MouseMesh->Render();

		m_MouseMaterial->ResetMaterial();
	}
}
...
void CInput::UpdateMouse(float DeltaTime)
{
	...
	// y좌표는 Top, Bottom이 뒤집어져야 하기 때문에(LB이 0, 0 좌표임) 계산해준다.
