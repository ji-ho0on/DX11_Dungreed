...
bool CSceneResource::CreateMesh(EMeshType Type, const std::string& Name, void* VtxData, int Size, int Count,
	...
	D3D11_USAGE IdxUsage, DXGI_FORMAT Fmt)
{
	...
	if (!CResourceManager::GetInst()->CreateMesh(m_Owner, Type, Name, VtxData, Size, Count,	VtxUsage, Primitive,
...
bool CSceneResource::CreateFontCollection(const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
	...
	CFontCollection* FontCollection = CResourceManager::GetInst()->FindFontCollection(Name);
