...
void CSceneCollision::DeleteCollider(CCollider* Collider)
{
...
{
	// 마우스와 충돌되었던 충돌체가 있었는데 사라졌다면 Release 콜백함수 호출 및 제거
	if (m_MouseCollision)
	{
	...
	// UI와 마우스의 충돌을 먼저 수행하고, 월드물체와 충돌을 시작한다.
	bool MouseWorldCollision = false;	// 충돌체와 충돌되었는지 판단

	// UI와 충돌되었는지 충돌 확인


	// UI와 충돌되지 않았다면 충돌체들과 충돌 확인
	if (!m_CollisionWidget)
	{
		...
		// Index가 제대로 들어왔다면 Section2D의 Index좌표에 해당하는 영역과 마우스 충돌을 하여 충돌된 물체가 있는지 확인한다.
		if (IndexX != -1 && IndexY != -1)
		{
			...
			// 충돌된 물체가 있다면 이미 마우스와 충돌하고 있는 충돌체인지 확인한다.
			if (ColliderMouse)
			{
				// 이미 마우스와 충돌하고있던 충돌체가 아니라면 Collision 콜백함수 호출
				if (ColliderMouse != m_MouseCollision)
					ColliderMouse->CallCollisionMouseCallback(ECollision_Result::Collision);

				// 이미 마우스와 충돌하고 있던 충돌체가 있고, 지금 충돌된 충돌체와 다르다면 이미 충돌하고 있던 충돌체의 Release 콜백함수를 호출
				if (m_MouseCollision && m_MouseCollision != ColliderMouse)
				{
					m_MouseCollision->CallCollisionMouseCallback(ECollision_Result::Release);
				}

				// 마우스와 충돌하고 있는 충돌체를 바꿔주고, 마우스가 충돌체와 충돌되었다고 처리한다.
	...
	// 마우스와 UI가 충돌되었고, 기존에 충돌되고 있던 충돌체가 있을 경우 Release 콜백함수를 호출하고 충돌 해제한다.
	...
	// 만약 마우스가 충돌체와 충돌되지 않았는데 이미 충돌하고 있던 충돌체가 있다면 Release 콜백함수를 호출하고 충돌 해제한다.
...
bool CSceneCollision::CreatePixelCollision(const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
...
bool CSceneCollision::CreatePixelCollisionMultibyte(const std::string& Name, const char* FileName, const std::string& PathName)
{
	const PathInfo* Path = CPathManager::GetInst()->FindPath(PathName);
