...
public:
	...
	template <typename T>
	static T* CreateSceneInfoCDO(const std::string& Name)
	{
		T* CDO = new T;

		AddSceneInfoCDO(Name, (CSceneInfo*)CDO);
	...
	template <typename T>
	static T* CreateAnimationCDO(const std::string& Name)
	{
		T* CDO = new T;

		CDO->Init();

		CAnimation2D::AddAnimationCDO(Name, (CAnimation2D*)CDO);
...
public:
	bool SetSceneInfoByCDO(CSceneInfo* SceneInfo);
	void CreateObjectByCDO(const std::string& Name, class CGameObject* GameObject);
