...
CEngine::CEngine()  :
    ...
    m_hWnd(0),
    m_WindowRS{},
    m_DeltaTimeScale(1.f),
    //m_ClearColor{0.f, 0.f, 0.f, 1.f}
    m_ClearColor{121 / 255.f , 185 / 255.f, 255 / 255.f}   // BackBuffer를 Clear할 색을 지정한다.
{
    _CrtSetDbgFlag(_CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF);
    //_CrtSetBreakAlloc(27020);
}
...
bool CEngine::Init(HINSTANCE hInst, const TCHAR* Title, const TCHAR* ClassName,
    int IconID, int SmallIconID, unsigned int WindowWidth, unsigned int WindowHeight,
    unsigned int DeviceWidth, unsigned int DeviceHeight, bool WindowMode, int MenuID)
{
    m_hInst = hInst;
    ...
    // EditorMode일 때 EditorGUIManager 초기화
    if (m_EditorMode)
    {
...
int CEngine::Run()
{
    ...
    while (m_Loop)
    {
        // PeekMessage : 이 함수도 메세지 큐에서 메세지를 꺼내오는 함수이다.
        // 단, 이 함수는 메세지 큐가 비어있을 경우 false를 반환하며 바로 빠져나오게 된다.
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))
        {
            // 메세지큐에서 꺼내온 메세지를 TranslateMessage 함수로 넘겨주면 문자 키인지 F1, 방향키 같은 키인지를 판단해준다.
            // 이러한 키들은 WM_KEYDOWN으로 메세지가 인식이 되고 문자 키는 WM_CHAR로 인식이 된다.
            // 키를 누르면 문자키의 경우 WM_CHAR도 만들어져야하기 때문에 WM_KEYDOWN이 일어나면 문자키의 경우
            // WM_CHAR메세지를 추가로 만들어서 메세지 큐에 넣어준다.
            TranslateMessage(&msg);

            // DispatchMessage 함수는 메세지큐에서 꺼내온 메세지를 메세지 처리 함수에 보내준다.
            // WndProc로 보내주는 것이다.
            DispatchMessage(&msg);
        }

        // 메세지 큐가 비어있을 경우 동작된다.
        // 즉, 윈도우의 데드타임시 이 else 구문이 동작되기 때문에 여기에 실제 게임로직을 작성하면 된다.
        else
        {
            Logic();
        }
...
void CEngine::Logic()
{
    // 타이머를 업데이트하여 델타타임을 가져옴
    m_Timer->Update();

    float DeltaTime = m_Timer->GetDeltaTime();

    DeltaTime *= m_DeltaTimeScale;
...
bool CEngine::Create(const TCHAR* Title, const TCHAR* ClassName)
{
    ...
    // 위에서 지정한 크기만큼 클라이언트 영역의 크기로 잡기 위해서 필요한 실제 윈도우의 크기를 얻어온다.
    ...
    // 윈도우 창을 보여준다. 1번인자에 들어간 핸들의 윈도우 창을 보여줄지 말지를 결정해준다.
    ShowWindow(m_hWnd, SW_SHOW);

    // 이 함수를 호출하여 클라이언트 영역이 제대로 갱신되었다면 0이 아닌 값을 반환하고 갱신이 실패했을 경우 0을 반환한다.
    UpdateWindow(m_hWnd);
...
{
    // EditorMode일 경우 ImGui용 WndProc에 넘김
    if (m_EditorMode)
    {
    ...
    // m_WndProcFunc가 있을 경우 해당 WndProc함수로 메세지 처리
    if (m_WndProcFunc)
    {
