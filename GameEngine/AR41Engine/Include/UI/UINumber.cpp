...
bool CUINumber::SetTextureArray(const std::string& Name, const std::vector<const TCHAR*>& vecFileName, const std::string& PathName)
{
    if (m_Scene)
    {
...
bool CUINumber::SetTextureArrayFullPath(const std::string& Name, const std::vector<const TCHAR*>& vecFullPath)
{
    if (m_Scene)
    {
...
void CUINumber::SetLoop(bool Loop)
{
    m_TextureInfo.Loop = Loop;
}

void CUINumber::SetReverse(bool Reverse)
{
...
void CUINumber::Render()
{
    ...
    for (size_t i = 0; i < Size; ++i)
    {
        ...
        // 출력할 때 다음 숫자는 이전 숫자로부터 5픽셀 떨어진 위치에서 출력한다.
        Space = m_Size.x + 5.f;
