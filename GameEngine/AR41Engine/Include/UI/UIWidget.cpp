    ...
    CRef(Widget)
{
    ...
    m_WidgetTypeName = Widget.m_WidgetTypeName;
    m_MouseHovered = false;
}
...
{
    // 출력될 위치정보를 만들어준다. m_Pos는 위젯을 가지고 있는 윈도우로부터 상대적인 위치정보로 구성된다.
    // 다른 UIWidget들에서 마우스 위치와의 관계를 설정할 때 RenderPos가 필요하므로 미리 구해준다.
...
void CUIWidget::Render()
{
    ...
    // UI는 기본방향으로 출력
    m_AnimCBuffer->SetDir((int)EDirection::Right);
