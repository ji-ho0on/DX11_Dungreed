...
bool CUIImage::SetTexture(const std::string& Name, const std::vector<const TCHAR*>& vecFileName, const std::string& PathName)
{
    if (m_Scene)
    {
...
bool CUIImage::SetTextureArray(const std::string& Name, const std::vector<const TCHAR*>& vecFileName, const std::string& PathName)
{
    if (m_Scene)
    {
...
bool CUIImage::SetTextureArrayFullPath(const std::string& Name, const std::vector<const TCHAR*>& vecFullPath)
{
    if (m_Scene)
    {
...
void CUIImage::AddFrameData(const Vector2& Start, const Vector2& End)
{
    ...
    m_TextureInfo.FrameTime = m_TextureInfo.PlayTime / m_TextureInfo.vecFrameData.size();
}
...
void CUIImage::SetLoop(bool Loop)
{
    m_TextureInfo.Loop = Loop;
}

void CUIImage::SetReverse(bool Reverse)
{
    m_TextureInfo.Reverse = Reverse;
}
...
void CUIImage::Update(float DeltaTime)
{
    ...
    {
        m_TextureInfo.Time += DeltaTime * m_TextureInfo.PlayScale;
        ...
        if (m_TextureInfo.Time >= m_TextureInfo.FrameTime)
        {
            ...
            if (m_TextureInfo.Reverse)
            {
                --m_TextureInfo.Frame;

                if (m_TextureInfo.Frame < 0)
                    AnimEnd = true;
            }
            ...
            else
            {
                ...
                if (m_TextureInfo.Frame == (int)m_TextureInfo.vecFrameData.size())
                    AnimEnd = true;
            }
        }
        ...
        if (AnimEnd)
        {
            ...
            {
                if (m_TextureInfo.Reverse)
                    m_TextureInfo.Frame = (int)m_TextureInfo.vecFrameData.size() - 1;
...
void CUIImage::Render()
{
    ...
    if (TextureEnable)
    {
        if (!m_TextureInfo.vecFrameData.empty())
        {
            ...
            m_AnimCBuffer->SetImageFrame(m_TextureInfo.vecFrameData[m_TextureInfo.Frame].Start, m_TextureInfo.vecFrameData[m_TextureInfo.Frame].End);
