...
#include <algorithm>
#include <stack>
#include <d3d11.h>			// DirectX 3D 11 헤더
#include <d3dcompiler.h>	// 쉐이더 컴파일을 위한 헤더
#include <dinput.h>			// dinput 사용을 위한 헤더
#include <filesystem>		// 파일 이름을 읽어오기 위한 헤더
#include <dwrite_3.h>
#include <d2d1.h>		
#include <process.h>		// Thread를 생성하기 위한 헤더
...
struct TransformCBuffer		// Transform용 상수 버퍼 구조체
{
	Matrix  matWorld;
	Matrix  matView;
	Matrix  matProj;
...
struct ColliderLine2DCBuffer
{
	Vector4	Color;
	Matrix	matWVP;
	Vector3 ConvertPoint;
	float	Empty;
};
...
{
	int				Header;		// 해당 데이터가 어떤 종류인지를 나타냄 (헤더에 따라 어떤 데이터 타입인지, 사이즈가 어떤지 등을 나타낸다)
	int				Size;		// 아래의 데이터 배열에 몇 바이트를 채웠는지를 나타낼 사이즈
	unsigned char	Data[1024];	// 데이터가 1바이트 단위로 들어갈 배열
};
